...
function safeText(value: string, limit = 1500) {
...
function issueJson(issue: AnalyzerViolation) {
  return {
    ...
    ruleId: issue.id,
    impact: issue.impact,
    priorityScore: priorityWeights[issue.impact],
    help: issue.help,
    descriptionFa: issue.description,
    wcagTags: issue.wcag,
    viewports: issue.viewportId ? [issue.viewportId] : [],
...
export function exportFileName(report: AnalysisReport, format: ExportFormat) {
  const prefix =
    format === "csv" ? "accessibility-issues" : "accessibility-report";
  ...
  return JSON.stringify(
    {
      ...
      report: {
        ...
        completedAt: report.analyzedAt,
        score: report.score,
        summary: report.summary,
        viewports: report.viewportReports,
        positivePoints: (report.positivePoints ?? []).map((point) => ({
          id: point.id,
          title: point.title,
          description: point.description,
          wcagTags: point.wcag,
          viewports: point.viewportId ? [point.viewportId] : [],
        })),
        issues: report.violations.map(issueJson),
      },
    },
    null,
    2,
...
export function createCsv(report: AnalysisReport) {
  ...
  const rows = report.violations.flatMap((issue) =>
