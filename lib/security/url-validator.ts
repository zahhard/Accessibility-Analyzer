...
export async function validatePublicUrl(raw: string): Promise<URL> {
  ...
  if (!["http:", "https:"].includes(parsed.protocol) || parsed.username || parsed.password ||
      parsed.hostname.length > 253 || localHosts.has(parsed.hostname.toLowerCase()) ||
      (net.isIP(parsed.hostname) > 0 && privateIp(parsed.hostname))) throw new Error("BLOCKED_URL");
  const records = await dns.lookup(parsed.hostname, { all: true, verbatim: true });
  if (!records.length || records.some(({ address }) => privateIp(address))) throw new Error("BLOCKED_URL");
  return parsed;
}
